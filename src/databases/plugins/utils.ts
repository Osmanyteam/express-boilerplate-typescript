...
MongoosePrivateUtils.prototype.updatePaths = function (paths, options) {
  ...
  return !options ? paths : _.merge({}, paths, this.toPaths(options.omit, true), this.toPaths(options.pick, false));
};
...
MongoosePrivateUtils.prototype.nestedOmit = function nestedOmit(doc, paths) {
  return _.reduce(
    doc,
    function (json, docValue, pathName) {
      const omit = paths[pathName];
      let t: any;
...
MongoosePrivateUtils.prototype.getPaths = function getPaths(schema, options) {
  ...
  const paths = _.reduce(
    schema.paths,
    function (result, pathSchema, path) {
      // Nested schema
      if (pathSchema.schema) {
        const opts = _.merge({ _parents: path }, options);
        const nestedPaths = self.getPaths(pathSchema.schema, opts);
      ...
      if (
        ...
        (self.include(pathSchema, path, options) || self.include({ options: pathSchema.options.type[0] }, path, options))
      ) {
        result[path] = true;
        return result;
      }
